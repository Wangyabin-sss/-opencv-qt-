...
{
    ui->setupUi(this);
    cv::Mat src = cv::imread("/home/ubuntu/桌面/qt/image/car.jpg");
    if (!src.data) {
    ...
    cv::threshold(hsvchannels[1],Threshold,150, 255, cv::THRESH_BINARY);
    ...
    //ui->label->setPixmap(QPixmap::fromImage(temp).scaled(ui->label->size(),Qt::KeepAspectRatio,Qt::SmoothTransformation));
